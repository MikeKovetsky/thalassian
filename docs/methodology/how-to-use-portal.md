...
Instead of manual copy-pasting, the agent dynamically spawned background subagents (isolated execution threads) to scour the web:
- **Wayback Machine Integration:** The agent recognized that current wikis lacked origin context. It autonomously scraped the *2006 World of Warcraft Encyclopedia* via `web.archive.org` using `curl` and `grep` to establish the foundational canon roots.
- **WoW: Midnight (12.0) Parsing:** It spawned four concurrent subagents to scrape Reddit (`r/wow`), MMO-Champion, and Wowhead datamining posts to extract brand-new, unreleased vocabulary (like `Shal'na` and `Anu'shalla`).
...
To generate the 100-word Swadesh list expansion, the primary agent (running on an efficient model) spawned an isolated subagent running **Claude 3 Opus**. The Opus agent was provided with the canonical roots and instructed to act as an anthropologist, using Markov-chain phonotactics to mathematically extrapolate new roots (e.g., `lura'sin` for *drink*). 

## 3. Sandboxed Code Execution & Test-Driven Development (TDD)
The agent wrote, executed, and debugged Python code natively within its isolated Linux workspace.
...
- **Automated Testing:** The agent proactively wrote `unittest` suites (`test_grammar.py`), executed them in the sandbox, read the traceback errors (like failing on irregular plurals like *phoenixes*), and autonomously patched the parser until all tests passed.

## 4. "Zero-Touch" Deployment Pipeline
The human operator never touched `git`. The agent handled the entire CI/CD lifecycle:
- Initialized the Git repository.
- Wrote a GitHub Actions workflow (`static.yml`) for deployment.
- Deployed the Web UI directly to GitHub Pages.
- Handled CSS/UI bugs on the fly (e.g., fixing mobile flexbox wrapping based on a user-provided screenshot).

## 5. Security-Conscious Prompt Engineering (Mode 1 & 3)
